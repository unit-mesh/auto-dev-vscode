...
export class CrossFileContextAnalyzer {
	private symbolLinkResolver = new SymbolLinkResolver(
		new BasedSymbolUsageFinder(),
		new LocationLinkResolver()
	);
	...
	async analyzeReferencedSymbols(
		...
		const links = await this.symbolLinkResolver.resolveImports(path, 30);

		const t1 = performance.now();

		console.info(
			"(context): " + (t1 - t0).toFixed(2) + "ms to resolve symbol links"
		);
	...
	getLinksToResolve(links: ILocationLink[]) {
		return uniqBy(
			links.filter(this.shouldKeepLocationLink),
			(link) => link.targetUri + "-" + serializePosition(link.targetRange.start)
		);
	}

	shouldKeepLocationLink(link: ILocationLink) {
		const originUri = link.originUri;
		const targetUri = link.targetUri;

		if (originUri === targetUri || "python" === inferLanguage(originUri)) {
			return false;
		}

		return (
			!targetUri.includes("builtins.pyi") &&
			!targetUri.includes("typeshed-fallback") &&
			(!!targetUri.includes("site-packages") ||
				(!targetUri.includes("lib/python") &&
					!targetUri.includes("lib\\python")))
		);
	}
