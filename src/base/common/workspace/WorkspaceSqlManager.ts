...
export class WorkspaceSqlManager {
                ...
                console.error(err.message);
            } else
						{
							this.db.run(`
								...
								VALUES (?, ?, ?, ?, ?, ?)
						`, [defaultData.id, defaultData.language, defaultData.code, defaultData.doc, defaultData.filePath, defaultData.codeContext]);
    ...
    public HasDataStorage(language: string, data: IDataStorage): Promise<boolean> {
        return new Promise((resolve, reject) => {
            ...
            this.db.get(`SELECT * FROM ${tableName} WHERE language = ? AND id = ?`, [language, dataInstance.id], (err, row) => {
                if (err) {
                    reject(err);
                } else {
    ...
    public GetDataStorageId(language: string, data: IDataStorage): Promise<number | undefined> {
        return new Promise((resolve, reject) => {
            ...
            this.db.get(`SELECT id FROM ${tableName} WHERE language = ? AND code = ? AND doc = ? AND filePath = ? AND codeContext = ?`, [language, dataInstance.code, dataInstance.doc, dataInstance.filePath, dataInstance.codeContext], (err, row) => {
                if (err) {
                    reject(err);
                } else {
    ...
    public ChangeDataStorage(language: string, oldDataStorage: IDataStorage, newDataStorage: IDataStorage): Promise<boolean> {
        return new Promise((resolve, reject) => {
            ...
            this.db.run(`UPDATE ${tableName} SET code = ?, doc = ?, filePath = ?, codeContext = ? WHERE language = ? AND id = ?`, [newDataInstance.code, newDataInstance.doc, newDataInstance.filePath, newDataInstance.codeContext, language, oldDataInstance.id], function (err) {
                if (err) {
                    reject(err);
                } else {
                    resolve(this.changes > 0);
		...
		public SaveDataStorage(language: string, data: IDataStorage): Promise<number> {
			return new Promise((resolve, reject) => {
					...
					// 检查数据是否已经存在
					this.GetDataStorageId(language, data).then(existingId => {
							if (existingId !== undefined) {
									resolve(existingId);
							} else {
									// 插入新数据
									this.db.run(`INSERT INTO ${tableName} (language, code, doc, filePath, codeContext) VALUES (?, ?, ?, ?, ?)`, [language, dataInstance.code, dataInstance.doc, dataInstance.filePath, dataInstance.codeContext], function (err) {
											if (err) {
													reject(err);
											} else {
													resolve(this.lastID);
