...
export const stateSlice = createSlice({
  ...
  initialState,
  reducers: {
    setConfig: (state, { payload: config }: PayloadAction<BrowserSerializedContinueConfig>) => {
      const defaultModelTitle =
        config.models.find((model) => model.title === state.defaultModelTitle)?.title ||
    ...
    setEditingContextItemAtIndex: (
      state,
      { payload: { index, item } }: PayloadAction<{ index?: number; item: ContextItemWithId }>
    ) => {
      if (index === undefined) {
        const isFirstContextItem = state.contextItems[0]?.id.itemId === item.id.itemId

        state.contextItems = isFirstContextItem ? [] : [{ ...item, editing: true }]
        return
      }
      // TODO
    },
    ...
    streamUpdate: (state, action: PayloadAction<string>) => {
      if (state.history.length) {
        state.history[state.history.length - 1].message.content += action.payload
      }
    },
    newSession: (state, { payload }: PayloadAction<PersistedSessionInfo | undefined>) => {
      if (payload) {
    ...
    deleteContextWithIds: (
      state,
      { payload }: PayloadAction<{ ids: ContextItemId[]; index: number | undefined }>
    ) => {
      const getKey = (id: ContextItemId) => `${id.providerTitle}-${id.itemId}`
      const ids = new Set(payload.ids.map(getKey))

      if (payload.index === undefined) {
        state.contextItems = state.contextItems.filter((item) => !ids.has(getKey(item.id)))
      } else {
    ...
    addHighlightedCode: (
      ...
      const base = payload.rangeInFileWithContents.filepath.split(/[\\/]/).pop()
    ...
    setDefaultModel: (state, { payload }: PayloadAction<string>) => {
      const model = state.config.models.find((model) => model.title === payload)
      if (!model) return
      return {
        ...state,
